...
void AudioBase::audioOut(int16_t * V, int32_t amount, int32_t rate)
{
  ...
  switch (rate)
  {
  case 16000: audioOut_16000(V, amount / 2);
    return;
  case 24000: audioOut_24000(V, amount / 2);
    return;
  case 32000: audioOut_32000(V, amount / 2);
    return;
  default:
  case 48000: audioOut_48000(V, amount / 2);
    return;
  }
}
...
void AudioBase::audioOut_16000(int16_t * V, int32_t amount)
{
  ...
  for (i = 0; i < amount; i++)
  {
    if (converter_16.convert(cmplx(V[2 * i + 0] / 32767.0, V[2 * i + 1] / 32767.0), outputBuffer, &result))
    {
      for (j = 0; j < result; j++)
      {
        buffer[2 * j + 0] = real(outputBuffer[j]);
...
void AudioBase::audioOut_24000(int16_t * V, int32_t amount)
{
  ...
  for (i = 0; i < amount; i++)
  {
    if (converter_24.convert(cmplx(V[2 * i + 0] / 32767.0, V[2 * i + 1] / 32767.0), outputBuffer, &result))
    {
      for (j = 0; j < result; j++)
      {
        buffer[2 * j + 0] = real(outputBuffer[j]);
...
void AudioBase::audioOut_32000(int16_t * V, int32_t amount)
{
  ...
  for (i = 0; i < amount; i++)
  {
    if (converter_32.convert(cmplx(V[2 * i + 0] / 32767.0, V[2 * i + 1] / 32767.0), outputBuffer, &result))
    {
      for (j = 0; j < result; j++)
      {
        buffer[2 * j + 0] = real(outputBuffer[j]);
...
{
  float * buffer = make_vla(float, 2 * amount * sizeof(float));
  int32_t i;

  for (i = 0; i < amount; i++)
  {
    buffer[2 * i + 0] = V[2 * i + 0] / 32767.0;
