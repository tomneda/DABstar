...
i32	plutoHandler::
	ad9361_set_trx_fir_enable (struct iio_device *dev, i32 enable) {
...
i32	plutoHandler::
	ad9361_get_trx_fir_enable (struct iio_device *dev, i32 *enable) {
bool value;
...
//extern "C" {
//i32 ad9361_set_bb_rate_custom_filter_manual(struct iio_device *dev,
//                                                  u32 rate, u32 Fpass,
...
//}
	...
	plutoSettings -> beginGroup ("plutoSettings");
        i32 x   = plutoSettings -> value ("position-x", 100). toInt ();
        i32 y   = plutoSettings -> value ("position-y", 100). toInt ();
	...
	if (Handle == NULL) {
	  throw (std_exception_string ("Failed to libiio.dll"));
	}
#else
	Handle		= dlopen ("libiio.so", RTLD_NOW);
	if (Handle == NULL) {
	   throw (std_exception_string ("we could not load libiio.so"));
	}
#endif
	...
	if (!success) {
           ...
           dlclose (Handle);
#endif
	   throw (std_exception_string ("could load all required lib functions"));
        }
...
//
	...
	if (iio_context_get_devices_count (ctx) <= 0) {
	   throw (std_exception_string ("no pluto devices detected"));
	}

	if (!get_ad9361_stream_dev (ctx, TX, &tx)) {
           throw (std_exception_string ("No TX device found"));
        }
...
//	   fprintf (stderr, "cound not obtain TX channel\n");
	   ...
	   throw (std_exception_string ("TX port 0 not found"));
        }
	...
	if (!get_ad9361_stream_ch (ctx, TX, tx, 0, &tx0_i)) {
           throw (std_exception_string ("TX chan i not found"));
        }

        if (!get_ad9361_stream_ch(ctx, TX, tx, 1, &tx0_q)) {
           throw (std_exception_string ("TX chan q not found"));
        }
	...
	if (txbuf == nullptr) {
	   iio_context_destroy (ctx);
	   throw (std_exception_string ("could not create TX buffer"));
	}
...
struct  iio_channel *lo_channel;
	...
	this -> fmFrequency = freq * KHz (1);
	  i32 ret = iio_channel_attr_write_longlong
                                     (lo_channel,
...
i32	p_inc		= iio_buffer_step          (txbuf);
i32	bufferLength	= i32 (p_end - p_begin);
i32	sourceSize	= bufferLength / (2 * sizeof (i16));
	...
	while (transmitting) {
	   ...
	   i32  nbytes_tx	= iio_buffer_push (txbuf);
	}
}
