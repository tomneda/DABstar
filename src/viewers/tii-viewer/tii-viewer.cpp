...
TiiViewer::TiiViewer(RadioInterface * mr, QSettings * dabSettings, RingBuffer<cmplx> * sbuffer) :
  myFrame(nullptr),
  spectrumCurve(""),
...
void TiiViewer::showSpectrum(int32_t amount)
{
  ...
  for (i = 0; i < TII_DISPLAYSIZE / 2; i++)
  {
    ...
    Y_values[TII_DISPLAYSIZE / 2 + i] = f / (TII_SPECTRUMSIZE / TII_DISPLAYSIZE);
    f = 0;
    for (j = 0; j < TII_SPECTRUMSIZE / TII_DISPLAYSIZE; j++)
    {
      f += abs(spectrumBuffer[TII_SPECTRUMSIZE / 2 + TII_SPECTRUMSIZE / TII_DISPLAYSIZE * i + j]);
    }
...
void TiiViewer::ViewSpectrum(double * X_axis, double * Y1_value, double amp, int32_t marker)
{
  uint16_t i;
  float amp1 = amp / 100;

  amp = amp / 50.0 * (-get_db(0));
  plotgrid->setAxisScale(QwtPlot::xBottom, (double)X_axis[0], X_axis[TII_DISPLAYSIZE - 1]);
  plotgrid->enableAxis(QwtPlot::xBottom);
  plotgrid->setAxisScale(QwtPlot::yLeft, get_db(0), get_db(0) + amp1 * 40);
...
void TiiViewer::show_nullPeriod(const QVector<float> & v, double amp2)
{
  ...
  plotgrid->setAxisScale(QwtPlot::xBottom, (double)X_axis[0], X_axis[TII_DISPLAYSIZE - 1]);
  plotgrid->enableAxis(QwtPlot::xBottom);
  plotgrid->setAxisScale(QwtPlot::yLeft, 0, Max * 2);
  //                                       get_db (0), 0);
  //	for (int i = 0; i < TII_DISPLAYSIZE; i ++)
