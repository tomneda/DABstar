...
RtlTcpClient::RtlTcpClient(QSettings * s) :
  myFrame(nullptr)
{
  remoteSettings = s;
  ...
  connect(tcp_ppm, SIGNAL (valueChanged(int)), this, SLOT (set_fCorrection(int)));
  connect(khzOffset, SIGNAL (valueChanged(int)), this, SLOT (set_Offset(int)));
...
void RtlTcpClient::wantConnect()
{
  ...
  connect(hostLineEdit, SIGNAL (returnPressed(void)), this, SLOT (setConnection(void)));
}
...
void RtlTcpClient::setConnection()
{
  QString s = hostLineEdit->text();
  QHostAddress theAddress = QHostAddress(s);

  serverAddress = QHostAddress(s);
  disconnect(hostLineEdit, SIGNAL (returnPressed(void)), this, SLOT (setConnection(void)));
...
void RtlTcpClient::sendRate(int32_t theRate)
{
  sendCommand(0x02, theRate);
}

void RtlTcpClient::setGainMode(int32_t gainMode)
{
...
void RtlTcpClient::set_fCorrection(int32_t ppm)
{
...
void RtlTcpClient::set_Offset(int32_t o)
{
