...
CATEGORY_BEGIN_NEW_NAME(Config, "Configuration")  // provide the name "Configuration" as category name in ini file instead of "Config"
  ...
  DEFINE_WIDGET(Config, cbUseNativeIqFormat)
  ...
  DEFINE_WIDGET(Config, cbDoDcAndIqCorr)
...
CATEGORY_BEGIN(EnsembleList)
  ...
  DEFINE_WIDGET(EnsembleList, cbShowELNoSignals)
  DEFINE_WIDGET(EnsembleList, cbShowELNewEntries)
  DEFINE_WIDGET(EnsembleList, cbShowELValidSignals)
