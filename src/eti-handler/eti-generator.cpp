...
void EtiGenerator::process_block(const std::vector<i16> & ibits, i32 iOfdmSymbIdx)
{
  ...
  if (iOfdmSymbIdx == 4)
  {
    ...
    Minor = 0;
    my_ficHandler->get_fib_decoder().getCIFcount(CIFCount_hi, CIFCount_lo);
  }
...
i32 EtiGenerator::_init_eti(u8 * oEti, i16 CIFCount_hi, i16 CIFCount_lo, i16 minor)
{
  ...
  for (i32 j = 0; j < 64; j++)
  {
    my_ficHandler->get_fib_decoder().getChannelInfo(&data, j);
    if (data.in_use)
    {
      NST++;
...
i32 EtiGenerator::_process_cif(const i16 * input, u8 * output, i32 offset)
{
  ...
  {
    ChannelData data;
    my_ficHandler->get_fib_decoder().getChannelInfo(&data, i);
    if (data.in_use)
    {
